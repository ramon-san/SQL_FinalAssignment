...
void search_controller_search(MYSQL *mysql, SEARCH_INFO info, int employee_id, char option){
    ...
    if(info.name[0] == '\0');
        ...
        sprintf(search, "name: %s", info.name);
        strcat(query, concat);
    ...
    if(info.father_surname[0] == '\0');
        ...
        sprintf(search, "father_surname: %s", info.father_surname);
        strcat(query, concat);
        if(info.field_number > 1){
            strcat(query, " AND ");
            strcat(search, ", ");
    ...
    if(info.mother_surname[0] == '\0');
        ...
        sprintf(search, "mother_surname: %s", info.mother_surname);
        strcat(query, concat);
        if(info.field_number > 1){
            strcat(query, " AND ");
            strcat(search, ", ");
    ...
    if(info.email[0] == '\0');
        ...
        sprintf(search, "email: %s", info.email);
        strcat(query, concat);
    }
    if(info.date_of_birth[0] == '\0');
    else{
        sprintf(concat, "date_of_birth = '%s'", info.date_of_birth);
        sprintf(search, "date_of_birth: %s", info.date_of_birth);
        strcat(query, concat);
        if(info.field_number > 1){
            strcat(query, " AND ");
            strcat(search, ", ");
    ...
    if(info.diagnosis[0] == '\0');
        ...
        sprintf(search, "diagnosis: %s", info.diagnosis);
        strcat(query, concat);
        if(info.field_number > 1){
            strcat(query, " AND ");
            strcat(search, ", ");
    ...
    if(info.date_of_consultation[0] == '\0');
        ...
        sprintf(search, "date_of_consultation: %s", info.date_of_consultation);
        strcat(query, concat);
    }
    if(option == 'P'){
